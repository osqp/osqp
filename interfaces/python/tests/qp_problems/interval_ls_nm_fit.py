...
def get_ratio_and_bounds(df):
    ...
    Compute (relative to the current group)
        ...
        2) ratio tr(P)/tr(A'A)
        3) ratio n / m 
        4) lower and upper bounds for rho (between 0 and 10)
    """
...
def get_grid_data(x, y, z, resX=200, resY=200):
...
for _, problem in tqdm(problems_p):
    ...
    A_temp = np.array([1., np.log(n), np.log(m)])
...
xi, yi, zi = get_grid_data(np.log10(res_p['noverm']),
                           np.log10(res_p['rho']),
                           res_p['scaled_iter'])
...
for _, problem in tqdm(problems_p):
    ...
    ratio_fit[i] = problem['noverm'].iloc[0]
