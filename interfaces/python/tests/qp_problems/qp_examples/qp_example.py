...
# Data structures
from utils.data_struct import data_struct
...
class QPExample(with_metaclass(abc.ABCMeta, object)):
    ...
    def perform_tests(self, **kwargs):
        ...
        tot_n_probs = self.dims_mat.shape[1] * \
            self.nm_num_prob * len(self.rho_vec) * \
        ...
        for i in range(self.dims_mat.shape[1]):
            for _ in range(self.nm_num_prob):  # Generate some random problems
                ...
                for rho in self.rho_vec:              # rho values
                    for sigma in self.sigma_vec:      # sigma values
                        for alpha in self.alpha_vec:  # alpha values
                            ...
                            data_str.update_data(current_seed,
