...
static char * tests_lin_alg()
{
    ...
    c_print("   2) Vector operations: ");
    tempflag = test_vec_operations();
    if (!tempflag) c_print("OK!\n");
    exitflag += tempflag;


    c_print("   3) Matrix operations: ");
    tempflag = test_mat_operations();
    if (!tempflag) c_print("OK!\n");
    exitflag += tempflag;

    c_print("   4) Matrix-vector multiplication: ");
    tempflag = test_mat_vec_multiplication();
    if (!tempflag) c_print("OK!\n");
    exitflag += tempflag;

    c_print("   5) Extract upper triangular: ");
    tempflag = test_extract_upper_triangular();
    if (!tempflag) c_print("OK!\n");
    exitflag += tempflag;

    c_print("   6) Compute QP form from upper triangular matrix: ");
